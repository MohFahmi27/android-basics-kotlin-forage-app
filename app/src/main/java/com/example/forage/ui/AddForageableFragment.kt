...
class AddForageableFragment : Fragment() {
    ...
    private val viewModel: ForageableViewModel by activityViewModels {
        ForageableViewModelFactory((requireActivity().application as BaseApplication).database.forageableDao())
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (id > 0) {
            ...
            viewModel.getForeable(id).observe(viewLifecycleOwner) { forageableResult ->
                forageable = forageableResult
                bindForageable(forageableResult)
            }
